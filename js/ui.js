...
async function renderApp(user) {
    ...
    if (missingContainer && missingText) {
        if (score < 100) {
            ...
            // Sanitize the missing details to prevent XSS
    ...
    if (nameDisplay) nameDisplay.innerText = sanitizeHTML(user.name || 'Nama');
    if (roleDisplay) {
        // Sanitize job title to prevent XSS
        const sanitizedJobTitle = sanitizeHTML(user.jobTitle || 'Role');
    ...
    if (compDisplay) {
        compDisplay.innerText = sanitizeHTML(user.company || '');
        compDisplay.classList.toggle('hidden', !user.company);
    }

    if (bioDisplay) bioDisplay.innerText = sanitizeHTML(user.bio || 'Belum ada bio.');
        ...
        if(!user.skills || user.skills.length === 0) {
            ...
            user.skills.forEach(s => {
                if (s && s.trim()) {
                    ...
                    skillsCont.innerHTML += `<span class="px-2 py-1 bg-slate-700 text-xs text-indigo-300 rounded border border-slate-600">${sanitizedSkill}</span>`;
                }
            });
        }
    }
