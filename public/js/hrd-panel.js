...
class HRDPanel {
  ...
  showCreateJobPosting() {
    ...
    setTimeout(() => {
      const container = document.getElementById('create-job-form-container');
      console.log('Create job container:', container);
      if (container) {
        this.renderCreateJobForm(container);
      } else {
        console.error('Create job form container not found!');
      }
    }, 100);
  }
  ...
  renderCreateJobForm(container) {
    if (!container) {
      console.error('renderCreateJobForm: container is null');
      return;
    }
    
    console.log('renderCreateJobForm: rendering form to container');
    ...
    container.innerHTML = `
      <form id="create-job-form">
        ...
        <div class="flex gap-4">
          <button type="submit" class="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg transition-all">
            Publikasikan Lowongan
          </button>
          <button type="button" onclick="router.navigate('hrd-my-jobs')" class="px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-lg transition-all">
            Batal
          </button>
        </div>
      </form>
    ...
    console.log('Form HTML injected, setting up event listener...');
...
document.addEventListener('DOMContentLoaded', () => {
  // Form will be handled by renderCreateJobForm
});